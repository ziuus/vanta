...
pub fn render(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    let mut lines: Vec<Line> = filtered.iter().rev().take(area.height.saturating_sub(2) as usize).rev().map(|l| {
    ...
    if app.panel_states.log_target_input_active {
        ...
        let input_block = Block::default()
            ...
            .title(Span::styled(" Filter Target (Enter to apply) ", Style::default().fg(t.accent)));
