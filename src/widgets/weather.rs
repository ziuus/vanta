...
pub fn render(f: &mut Frame, area: Rect, theme: &Theme) {
    ...
    if !snap.ready {
        lines.push(Line::from(vec![Span::styled("Fetching weather...", Style::default().fg(theme.dim))]));
    ...
    for i in 0..5 {
        ...
        spans.push(Span::styled(icon[i], Style::default().fg(if snap.is_day || i == 1 { theme.accent } else { theme.surface })));
