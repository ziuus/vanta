...
pub trait DashboardWidget {
    ...
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, config: &Config, states: &PanelStates, tick: u64);
}
...
impl DashboardWidget for CpuWidget {
    ...
    fn icon(&self) -> &'static str { "▂▁▃ " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, _states: &PanelStates, _tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); cpu::render(f, inner, theme);
    }
}
...
impl DashboardWidget for MemoryWidget {
    ...
    fn icon(&self) -> &'static str { "🧠 " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, _states: &PanelStates, _tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); memory::render(f, inner, theme);
    }
}
...
impl DashboardWidget for DiskWidget {
    ...
    fn icon(&self) -> &'static str { "💾 " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, _states: &PanelStates, _tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); disk::render(f, inner, theme);
    }
}
...
impl DashboardWidget for NetworkWidget {
    ...
    fn icon(&self) -> &'static str { "🌐 " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, _states: &PanelStates, _tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); network::render(f, inner, theme);
    }
}

impl DashboardWidget for GpuWidget {
    fn id(&self) -> &'static str { "gpu" }
    fn label(&self) -> &'static str { "GPU" }
    fn icon(&self) -> &'static str { "🎮 " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, _states: &PanelStates, _tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); gpu::render(f, inner, theme);
    }
}
...
impl DashboardWidget for SystemWidget {
    ...
    fn icon(&self) -> &'static str { "🖥 " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, _states: &PanelStates, _tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); system_info::render(f, inner, theme);
    }
}

impl DashboardWidget for ClockWidget {
    fn id(&self) -> &'static str { "clock" }
    fn label(&self) -> &'static str { "Clock" }
    fn icon(&self) -> &'static str { "⏱ " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, _states: &PanelStates, _tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); clock::render(f, inner, theme);
    }
}
...
impl DashboardWidget for CalendarWidget {
    ...
    fn icon(&self) -> &'static str { "📅 " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, states: &PanelStates, _tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); calendar::render(f, inner, theme, states.calendar_month_offset);
    }
}

impl DashboardWidget for MediaWidget {
    fn id(&self) -> &'static str { "media" }
    fn label(&self) -> &'static str { "Media" }
    fn icon(&self) -> &'static str { "🎵 " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, _states: &PanelStates, _tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); media::render(f, inner, theme);
    }
}

impl DashboardWidget for MusicVizWidget {
    fn id(&self) -> &'static str { "music_viz" }
    fn label(&self) -> &'static str { "Visualizer" }
    fn icon(&self) -> &'static str { "🎵 " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, _states: &PanelStates, tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); music_viz::render(f, inner, theme, tick);
    }
}

impl DashboardWidget for ProcessesWidget {
    fn id(&self) -> &'static str { "processes" }
    fn label(&self) -> &'static str { "Processes" }
    fn icon(&self) -> &'static str { "⚙ " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, _config: &Config, states: &PanelStates, _tick: u64) {
...
impl DashboardWidget for CmatrixWidget {
    fn id(&self) -> &'static str { "cmatrix" }
    fn label(&self) -> &'static str { "Cmatrix" }
    fn icon(&self) -> &'static str { "〰 " }
    fn render(&self, f: &mut Frame, area: Rect, _theme: &Theme, _config: &Config, _states: &PanelStates, tick: u64) {
        let inner = draw_border(f, area, self.label(), _theme); cmatrix::render(f, inner, tick);
    }
}
...
impl DashboardWidget for SettingsWidget {
    ...
    fn icon(&self) -> &'static str { "⚙ " }
    fn render(&self, f: &mut Frame, area: Rect, theme: &Theme, config: &Config, _states: &PanelStates, _tick: u64) {
        let inner = draw_border(f, area, self.label(), theme); settings::render(f, inner, theme, config);
    }
}
